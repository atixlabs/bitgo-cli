...
function loadJSON( name ) {
  try {
    data = fs.readFileSync( jsonFilename( name ), {
      encoding: 'utf8'
    } );
...
function saveJSON( name, data ) {
  ...
  fs.writeFileSync( jsonFilename( name ), data, {
    encoding: 'utf8',
    mode: 0600
  } );
}
...
BGCL.prototype.createArgumentParser = function() {
  ...
  login.addArgument( [ '-p', '--password' ], {
    help: 'the account password'
  } );
  login.addArgument( [ '-o', '--otp' ], {
    help: 'the 2-step verification code'
  } );
  ...
  token.addArgument( [ 'token' ], {
    nargs: '?',
    help: 'the token to set'
  } );
   ...
   * OTP Commands
   */
  var otp = subparsers.addParser( 'otp', {
    help: 'OTP commands (use otp -h to see commands)'
  } );
  ...
  // otp list
  var otpList = otpCommands.addParser( 'list', {
    help: 'List OTP methods'
  } );

  // otp remove
  var otpRemove = otpCommands.addParser( 'remove', {
    help: 'Remove OTP device'
  } );
  otpRemove.addArgument( [ 'deviceId' ], {
    help: 'the device id to remove'
  } );

  // otp add
  var otpAdd = otpCommands.addParser( 'add', {
    help: 'Add an OTP device'
  } );
  otpAdd.addArgument( [ 'type' ], {
    choices: [ 'totp', 'yubikey', 'authy' ],
    help: 'Type of device to add'
  } );
  ...
  wallets.addArgument( [ '-p', '--page' ], {
    nargs: '?',
    help: 'Number of page of size limit to skip'
  } );
  wallets.addArgument( [ '-l', '--limit' ], {
    nargs: '?',
    help: 'Limit wallet listing result'
  } );
  ...
  wallet.addArgument( [ 'wallet' ], {
    nargs: '?',
    help: 'the index, id, or name of the wallet to set as current'
  } );
  ...
  balance.addArgument( [ '-c', '--confirmed' ], {
    action: 'storeTrue',
    help: 'Exclude unconfirmed transactions'
  } );
  balance.addArgument( [ '-u', '--unit' ], {
    help: 'select units: satoshi | bits | btc [default]'
  } );
  ...
  setLabel.addArgument( [ 'address' ], {
    help: 'the address to label'
  } );
  setLabel.addArgument( [ 'label' ], {
    help: 'the label'
  } );
  ...
  removeLabel.addArgument( [ 'address' ], {
    help: 'the address from which to remove the label'
  } );
  ...
  addresses.addArgument( [ '-c', '--change' ], {
    action: 'storeTrue',
    help: 'include change addresses'
  } );
  ...
  newAddress.addArgument( [ '-c', '--change' ], {
    action: 'storeTrue',
    help: 'create a change address'
  } );
  newAddress.addArgument( [ '-l', '--label' ], {
    help: 'optional label'
  } );
  ...
  unspents.addArgument( [ '-c', '--minconf' ], {
    help: 'show only unspents with at least MINCONF confirms'
  } );
  ...
  consolidateUnspents.addArgument( [ '-t', '--target' ], {
    type: 'int',
    help: 'consolidate unspents until only TARGET number of unspents is left (defaults to 1)'
  } );
  consolidateUnspents.addArgument( [ '-i', '--inputCount' ], {
    type: 'int',
    help: 'use up to that many inputs in a consolidation batch (defaults to 85)'
  } );
  consolidateUnspents.addArgument( [ '-f', '--feeRate' ], {
    type: 'int',
    help: 'set fee rate in satoshis per KB'
  } );
  consolidateUnspents.addArgument( [ '-c', '--confirmTarget' ], {
    type: 'int',
    help: 'set fee based on estimates for getting confirmed within this number of blocks'
  } );
  ...
  fanoutUnspents.addArgument( [ '-t', '--target' ], {
    type: 'int',
    required: true,
    help: 'fan out up to TARGET number of unspents'
  } );
  ...
  sendToAddress.addArgument( [ '-d', '--dest' ], {
    help: 'the destination address'
  } );
  sendToAddress.addArgument( [ '-a', '--amount' ], {
    help: 'the amount in BTC'
  } );
  sendToAddress.addArgument( [ '-p', '--password' ], {
    help: 'the wallet password'
  } );
  sendToAddress.addArgument( [ '-o', '--otp' ], {
    help: 'the 2-step verification code'
  } );
  sendToAddress.addArgument( [ '-c', '--comment' ], {
    help: 'optional private comment'
  } );
  sendToAddress.addArgument( [ '-u', '--unconfirmed' ], {
    nargs: 0,
    help: 'allow spending unconfirmed external inputs'
  } );
  sendToAddress.addArgument( [ '--confirm' ], {
    action: 'storeConst',
    constant: 'go',
  ...
  freezeWallet.addArgument( [ '-d', '--duration' ], {
    help: 'the duration in seconds for which to freeze the wallet'
  } );
  ...
  removeWallet.addArgument( [ 'wallet' ], {
    nargs: '?',
    help: 'the wallet ID of the wallet (default: current)'
  } );
  ...
  shareWallet.addArgument( [ '-e', '--email' ], {
    help: "email address of the recipient's BitGo account"
  } );
  shareWallet.addArgument( [ '-r', '--role' ], {
    help: 'role for the recipient on this wallet',
    choices: [ 'admin', 'spender', 'viewer' ]
  } );
  shareWallet.addArgument( [ '-p', '--password' ], {
    help: "the wallet password"
  } );
  shareWallet.addArgument( [ '-o', '--otp' ], {
    help: 'the 2-step verification code'
  } );
  shareWallet.addArgument( [ '-c', '--comment' ], {
    help: 'a message for the recipient'
  } );
  shareWallet.addArgument( [ 'wallet' ], {
    nargs: '?',
    help: 'the wallet id to share (default: current)'
  } );
  ...
  newKey.addArgument( [ 'entropy' ], {
    nargs: '?',
    help: 'optional additional entropy'
  } );
  ...
  newWallet.addArgument( [ '-n', '--name' ], {
    help: 'name for the wallet'
  } );
  newWallet.addArgument( [ '-u', '--userkey' ], {
    help: 'xprv for the user keychain'
  } );
  newWallet.addArgument( [ '-b', '--backupkey' ], {
    help: 'xpub for the backup keychain'
  } );
  ...
  splitKeys.addArgument( [ '-m' ], {
    help: 'number of shares required to reconstruct a key'
  } );
  splitKeys.addArgument( [ '-n' ], {
    help: 'total number of shares per key'
  } );
  splitKeys.addArgument( [ '-N', '--nkeys' ], {
    help: 'total number of keys to generate'
  } );
  splitKeys.addArgument( [ '-p', '--prefix' ], {
    help: 'output file prefix'
  } );
  splitKeys.addArgument( [ '-e', '--entropy' ], {
    help: 'additional user-supplied entropy'
  } );
  ...
  verifySplitKeys.addArgument( [ '-f', '--file' ], {
    help: 'the input file (JSON format)'
  } );
  verifySplitKeys.addArgument( [ '-k', '--keys' ], {
    help: 'comma-separated list of key indices to recover'
  } );
  ...
  recoverKeys.addArgument( [ '-v', '--verifyonly' ], {
    action: 'storeConst',
    constant: 'true',
    help: 'verify only (do not show xprvs)'
  } );
  recoverKeys.addArgument( [ '-f', '--file' ], {
    help: 'the input file (JSON format)'
  } );
  recoverKeys.addArgument( [ '-k', '--keys' ], {
    help: 'comma-separated list of key indices to recover'
  } );
  ...
  dumpWalletUserKey.addArgument( [ '-p', '--password' ], {
    help: 'the wallet password'
  } );
  dumpWalletUserKey.addArgument( [ '--confirm' ], {
    action: 'storeConst',
    constant: 'go',
    help: 'skip interactive confirm step -- be careful!'
  } );
  ...
  createTx.addArgument( [ '-d', '--dest' ], {
    help: 'the destination address'
  } );
  createTx.addArgument( [ '-a', '--amount' ], {
    help: 'the amount in BTC'
  } );
  createTx.addArgument( [ '-f', '--fee' ], {
    help: 'fee to pay for transaction'
  } );
  createTx.addArgument( [ '-c', '--comment' ], {
    help: 'optional private comment'
  } );
  createTx.addArgument( [ '-p', '--prefix' ], {
    help: 'output file prefix'
  } );
  createTx.addArgument( [ '-u', '--unconfirmed' ], {
    nargs: 0,
    help: 'allow spending unconfirmed external inputs'
  } );
  ...
  signTx.addArgument( [ '-f', '--file' ], {
    help: 'the input transaction file (JSON format)'
  } );
  signTx.addArgument( [ '--confirm' ], {
    action: 'storeConst',
    constant: 'go',
    help: 'skip interactive confirm step -- be careful!'
  } );
  signTx.addArgument( [ '-k', '--key' ], {
    help: 'xprv (private key) for signing'
  } );
  signTx.addArgument( [ '-p', '--prefix' ], {
    nargs: '?',
  ...
  sendTransaction.addArgument( [ '-t', '--txhex' ], {
    help: 'the transaction hex to send'
  } );
  sendTransaction.addArgument( [ '-f', '--file' ], {
    nargs: '?',
    help: 'optional input file containing the tx hex'
  } );
  ...
  addWebhook.addArgument( [ '-u', '--url' ], {
    help: 'URL of new webhook'
  } );
  addWebhook.addArgument( [ '-n', '--numConfirmations' ], {
    help: 'Number of confirmations before calling webhook',
    defaultValue: 0
  } );
  addWebhook.addArgument( [ '-t', '--type' ], {
    help: 'Type of webhook: e.g. transaction',
  ...
  removeWebhook.addArgument( [ '-u', '--url' ], {
    help: 'URL of webhook to remove'
  } );
  removeWebhook.addArgument( [ '-t', '--type' ], {
    help: 'Type of webhook: e.g. transaction',
    defaultValue: 'transaction'
  } );
  ...
  var removeWhitepolicy = subparsers.addParser( 'removeWhitepolicy', {
    addHelp: true,
    help: 'Removes a whitepolicy rule from a wallet'
  } );
  ...
  recoverLitecoin.addArgument( [ '-i', '--inputaddresses' ], {
    help: 'JSON array of input addresses to obtain litecoin from'
  } );
  recoverLitecoin.addArgument( [ '-r', '--recipients' ], {
    help: 'JSON dictionary of recipients in { addr1: satoshis }'
  } );
  recoverLitecoin.addArgument( [ '-p', '--prefix' ], {
    help: 'optional output file prefix'
  } );
...
BGCL.prototype.handleOTPAddTOTP = function() {
  ...
  return this.doGet( '/user/otp/totp', {} )
    .then( function( res ) {
      key = res;
      fs.writeFileSync( svgFile, qr.imageSync( key.url, {
        type: 'svg'
      } ) );
...
BGCL.prototype.handleWallets = function( setWallet, page, limit ) {
  ...
  return self.bitgo.wallets().list( {
    ...
    .then( function( result ) {
      var wallets = result.wallets.filter( function( w ) {
        return w.type() !== 'coinbase';
      } );
    ...
    .then( function( wallets ) {
      var sortedWallets = _.sortBy( wallets, function( w ) {
        return w.label() + w.id();
      } );
...
BGCL.prototype.handleWallet = function() {
  ...
  return this.bitgo.wallets().get( {
      id: this.session.wallet.id()
    } )
    .then( function( result ) {
      wallet = result;
      wallet.validateAddress( {
        address: wallet.wallet.id,
        path: "/0/0"
      } );
...
BGCL.prototype.handleLabels = function() {
  ...
  // normal output
  var sortedWallets = _.sortBy( _.values( this.session.wallets ), function( w ) {
    return w.label + w.id;
  } );
  sortedWallets.forEach( function( wallet ) {
    var labels = labelsByWallet[ wallet.id ];
    if ( labels ) {
      var sortedLabels = _.sortBy( labels, function( label ) {
...
BGCL.prototype.handleAcceptShare = function() {
  ...
  return this.ensureAuthenticated()
    ...
    .then( function() {
      return self.bitgo.verifyPassword( {
...
BGCL.prototype.handleCancelShare = function() {
  ...
  return this.ensureAuthenticated()
    .then( function() {
      return self.bitgo.wallets().cancelShare( {
        walletShareId: input.share
      } );
    } )
    .then( function( result ) {
...
BGCL.prototype.handleFreezeWallet = function() {
  ...
  return this.ensureWallet()
    ...
    .then( function() {
      ...
      return self.session.wallet.freeze( {
...
BGCL.prototype.handleRemoveWallet = function() {
  ...
  return this.ensureWallet()
    ...
    .then( function() {
      ...
      return self.bitgo.wallets().remove( {
        id: walletId
      } );
    } )
    .then( function() {
...
BGCL.prototype.handleSendCoins = function() {
  ...
  var sendTransactionWithSeed = function( wallet, txParams, seed ) {
    const keychain = this.bitgo.keychains().create( {
      seed: new Buffer( seed, 'utf-8' )
    } )
...
BGCL.prototype.handleSignTx = function() {
  ...
  return Q()
    ...
    .then( function() {
      ...
      return self.bitgo.newWalletObject( {
        wallet: {
          id: params.walletId
        },
        private: {
          keychains: params.walletKeychains
        }
      } );
    } )
    .then( function( result ) {
...
BGCL.prototype.handleNewWallet = function() {
  ...
  return this.ensureAuthenticated()
    ...
    .then( function() {
      try {
        if ( input.backupkey.substr( 0, 4 ) !== 'xpub' ) {
          throw new Error();
        }
      ...
      .then( function() {
        ...
        shares.forEach( function( share, shareIndex ) {
          try {
            ...
            if ( !passwords.some( function( p ) {
                return p.shareIndex === shareIndex;
              } ) ) {
              passwords.push( {
...
BGCL.prototype.handleAddWhitepolicy = function() {
  var self = this;
  var wallet = this.session.wallet;
  var input = new UserInput( this.args );
  ...
  return input.getVariable( 'address', 'Address to add to wallet ' + wallet.id() + ' : ', true )()
    .then( () => {
      const policyData = {
        'action': {
          'type': 'deny'
        ...
        return request.get( url )
          .then( function( result ) {
            ...
            var results = inputAddresses.map( function( address ) {
              ...
              return {
                address: address,
                unspents: resultsByAddress[ address ].unspent
              };
            } );
            return results;
          } )
